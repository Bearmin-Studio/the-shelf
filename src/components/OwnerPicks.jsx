...
export default function OwnerPicks({ onSelect, picks, ownerIcon, ownerName }) {
  ...
  return (
    <section id="owner" className="bg-[var(--bg-secondary)] border-y border-[var(--border)] py-12">
      <div className="max-w-[1200px] mx-auto px-6">
        <div className="flex items-center gap-3 mb-5">
          {ownerIcon ? (
            <img src={ownerIcon} alt={displayName} className="w-9 h-9 rounded-full object-cover" />
          ) : (
            <div className="w-9 h-9 rounded-full bg-[var(--text-primary)] text-white flex items-center justify-center text-sm font-bold">
              {displayName[0]}
            </div>
          )}
          <div>
            <div className="text-sm font-semibold">{displayName}の今月の発見</div>
            <div className="text-xs text-[var(--text-tertiary)]">棚を見て回って、気になった工房をご紹介</div>
          </div>
        </div>
        <div className="flex gap-4 overflow-x-auto pb-2" style={{ scrollbarWidth: 'none' }}>
          {picksData.map(p => (
            <button key={p.factory?.id} onClick={() => onSelect(p.factory)}
              className="flex-shrink-0 w-[300px] text-left bg-white border border-[var(--border)] rounded-2xl p-5 hover:-translate-y-1 hover:shadow-[var(--shadow-md)] transition-all cursor-pointer">
